...
import { discountOption, discountType, options, quotationItems, quotationStatus, owner as companyEmployee } from '../../../core/common/selectoption/selectoption';
...
const CreditNotes = () => {
  ...
  const [selectedCreditNotesStatus, setSelectedCreditNotesStatus] = useState([]);
  ...
  const handleToggleCreditNotesColumnVisibility = (columnTitle) => {
    setCreditNotesColumnVisibility((prevVisibility) => ({
      ...prevVisibility,
  ...
  const filterCreditNotesStatus = (creditNotes) => {
    setSelectedCreditNotesStatus((prevStatus) =>
      prevStatus.includes(creditNotes)
        ? prevStatus.filter((status) => status !== creditNotes)
        : [...prevStatus, creditNotes]
    );
  };
  const resetFilters = () => {
    setSelectedCreditNotesStatus([]);
    setSearchQuery('');
  }
  const exportCreditNotesToPDF = () => {
    const doc = new jsPDF();

    const filteredColumns = creditNotesColumns.filter(
      (col) =>
        creditNotesColumnVisibility[col.title]
    );


    const headers = filteredColumns.map((col) => col.title);
    const data = filteredCreditNotesData.map((row) =>
      filteredColumns.map((col) => row[col.dataIndex] || "")
    );


    const pageWidth = doc.internal.pageSize.getWidth();
    const titleText = "Credit Notes Report";
  ...
  const creditNotesStatus = Object.values(
    creditNotesData.reduce((acc, creditNotes) => {
      if (!acc[creditNotes.status]) {
        acc[creditNotes.status] = { value: creditNotes.status, label: creditNotes.status };
      }
      return acc;
    }, {})
  );
  const filteredCreditNotesData = creditNotesData.filter((creditNotes) => {
    ...
    const matchesStatus =
      selectedCreditNotesStatus.length === 0 ||
      selectedCreditNotesStatus.includes(creditNotes.status.toLowerCase());
  ...
  console.log(selectedCreditNotesStatus,"selectedCreditNotesStatus");
      ...
      <div className="card">
        ...
        <div className="card-header">
          <div className="row align-items-center justify-content-end">
            {!showCreditNotesViewForm && !showCreditNotesForm && <div className="col-sm-12">
                          ...
                          <div className="filter-set-content">
                            ...
                            >
                              <div className="filter-content-list">
                                <ul>
                                  {creditNotesStatus.map((creditNotesStatus, index) => {
                                    return (
                                      <li
                                        key={index}
                                      >
                                        <div className="filter-checks" >
                                          <label className="checkboxs" >
                                            <input
                                              type="checkbox"
                                              checked={selectedCreditNotesStatus.includes(creditNotesStatus.value.toLowerCase())}
                                              onChange={() => filterCreditNotesStatus(creditNotesStatus.value.toLowerCase())}
                                            />
                                            <span className="checkmarks" />
                                            {creditNotesStatus.value}
                                          </label>
                                        </div>
                                      </li>
                                    );
                  ...
                  <div className="dropdown">
                    ...
                    <div className="dropdown-menu  dropdown-menu-md-end dropdown-md p-3">
                      ...
                      <div className="border-top pt-3">
                        {creditNotesColumns.map((column, index) => {
                          if (
                            column.title === ""
                          ) {
                            return;
                            ...
                            >
                              ...
                              >
                                <input
                                  ...
                                  defaultChecked={true}
                                  onClick={() =>
                                    handleToggleCreditNotesColumnVisibility(
                                      column.title
                                    )
                                  }
                                />
            ...
            {
              showCreditNotesForm && !showCreditNotesViewForm && <h4 className="fw-semibold mb-0">Credit Notes</h4>
            }
            {
              showCreditNotesViewForm && <div className="dropdown me-2" style={{ width: 'auto' }}>
                <Link
                  to="#"
                  className="dropdown-toggle"
                  data-bs-toggle="dropdown"
